...
# ==================================================================================
# RAVDESS: 0.5    ADFES:1 (1.25)   CremaD:0.5    SAVEE:1
frame_cut = 0
# ==================================================================================
...
#dataset_directory=['E:\\Unity\\EMPACTVR_Demos_Version1\\Assets\\StreamingAssets\\OpenCVForUnity\\emotion_videos\\ADFES_Labelled_MP4\\%s\\*.mp4']

dataset_directory=['E:\\Unity\\EMPACTVR_Demos_Version1\\Assets\\StreamingAssets\\OpenCVForUnity\\emotion_videos\\RAVDESS_640_Labelled\\%s\\*.mp4']
...
AUDIO_MODEL_PATH = '..\\unity_audio_model\\AudioClassifier_V2.onnx'
...
def extract_facial_feature(landmarks, image, w, h):
    ...
    AU2_r = (math.atan2(y[70] - y[63], x[63] - x[70]) + math.atan2(y[46] - y[53], x[53] - x[46]) + math.atan2(y[225] - y[224], x[224] - x[225]))
    #AU2_r = round(AU2_r, 3) 
    AU2_r = lerp(AU2_r,0.8,3)
    AU2_l = (math.atan2(y[300] - y[293],x[300] - x[293]) + math.atan2(y[276] - y[283],x[276] - x[283]) + math.atan2(y[445] - y[444],x[445] - x[444]))
    #AU2_l = round(AU2_l, 3) 
    AU2_l = lerp(AU2_l,0.8,3)

    #AU2 = round(((AU2_r + AU2_l) / 2), 3)
    AU2 = round((AU2_r + AU2_l), 3)
    ...
    AU7_r = ((distance(x[159],y[159],x[145],y[145]) + distance(x[159],y[159],x[145],y[145])) / (2 * distance(x[130],y[130],x[243],y[243])))
    #AU7_r = round(AU7_r, 3) 
    AU7_r = lerp(AU7_r,0.01,0.5)
    AU7_l = ((distance(x[386],y[386],x[374],y[374]) + distance(x[386],y[386],x[374],y[374])) / (2 * distance(x[463],y[463],x[359],y[359])))
    #AU7_l = round(AU7_l, 3) 
    AU7_l = lerp(AU7_l,0.01,0.5)
    
    AU7 = round(((AU7_r + AU7_l) / 2), 3)
    #AU7 = round((AU7_r + AU7_l), 3)
    ...
    AU15_r = math.atan2(y[43] - y[76], x[76] - x[43]) 
    #AU15_r = round(AU15_r, 3) 
    AU15_r = lerp(AU15_r,-1.1,2.25)
    AU15_l = math.atan2(y[273] - y[306],x[273] - x[306])
    #AU15_l = round(AU15_l, 3) 
    AU15_l = lerp(AU15_l,-1.1,2.25)
    
    #AU15 = round(((AU15_r + AU15_l) / 2), 3)
    AU15 = round((AU15_r + AU15_l), 3)
