...
def section_image_mean_prediction(file, interval, full_face_data,full_face_label):
    ...
    while samples_wrote < samples_total:
        ...
        if len(block) >= int(interval * fps):
            ...
            block_mean_pred = predict(temp, ort_session)
            #block_probability = facial_model.predict_proba(mean_array)
            #print(block_mean_pred)
            #mean_prob = np.mean(block_mean_pred, axis = 0)
            face_list.append(block_mean_pred)
    
    face_probability = np.array(face_list)
    print(face_probability)
    return face_probability    

def section_image_median_prediction(file, interval, full_face_data,full_face_label):
    ...
    while samples_wrote < samples_total:
        ...
        if len(block) >= int(interval * fps):
            ...
            median_array = np.median(block, axis=0)
            ...
            block_median_pred = predict(temp, ort_session)
            #block_probability = facial_model.predict_proba(mean_array)
            #print(block_mean_pred)
            #mean_prob = np.mean(block_mean_pred, axis = 0)
            face_list.append(block_median_pred)
    ...
    for emotion in observed_emotions:   
        ...
        for d_index,d_item in enumerate(dataset_directory):
            ...
            for file in glob.glob(d_item %emotion): 
                ...
                face_probability = section_image_median_prediction(file, interval, full_face_data,full_face_label)
                
                face_emotion_profile = [0, 0, 0, 0, 0, 0, 0]
                for i, value in enumerate(face_probability):
                    face_emotion_profile[value] += 1
                highest_face_index = np.argmax(face_emotion_profile) 
                print(observed_emotions[highest_face_index])
                print(highest_face_index, "-", index, " (",  face_emotion_profile[highest_face_index], "-", face_emotion_profile[index], ")")
                if face_emotion_profile[highest_face_index] == face_emotion_profile[index]:
                    highest_face_index = index
                #if observed_emotions[highest_face_index] == emotion:
                ...
                #    if highest_face_index == index:
                #        dataset_facial_correct[d_index][index] += 1;                         
                #    print("FACE:",face_emotion_profile, "  high=",str(observed_emotions[highest_face_index]))
